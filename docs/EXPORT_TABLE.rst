...
specified schema or the current schema if ASCHEMA is omitted. If the optional
INCLUDE_GENERATED parameter is 'Y' (the default), GENERATED ALWAYS columns will
be included, otherwise they are excluded. GENERATED BY DEFAULT columns are
always included. If the optional INCLUDE_IDENTITY parameter is 'Y' (the
default), IDENTITY columns will be included, otherwise they are excluded.

See the :ref:`EXPORT_SCHEMA` function for more information on the generated
command.
...
INCLUDE_GENERATED
    If this parameter is ``'Y'`` then any columns defined as ``GENERATED BY``
    in the source table will be included in the export. Contrariwise, if
    ``'N'``, generated columns will be excluded from the command. Defaults to
    ``'Y'`` if omitted.
INCLUDE_IDENTITY
