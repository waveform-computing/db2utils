...
``'MONTHLY'``, if RESOLUTION is ``'WEEK'``, or ``'WEEK_ISO'`` then the suffix
is ``'WEEKLY'`` and so on. If DEST_SCHEMA and SOURCE_SCHEMA are not specified
...
generated for the end of each week according to the :ref:`WEEK_END` function
from the earliest record in the history table up to the current date. See the
:ref:`CREATE_HISTORY_TRIGGERS` documentation for a list of the possible values.
...
RESOLUTION
    ...
    See the :ref:`CREATE_HISTORY_TRIGGERS` documentation for a list of possible
    values. This should be greater than or equal to the RESOLUTION specified
    when the source table was created with :ref:`CREATE_HISTORY_TABLE` (it is
